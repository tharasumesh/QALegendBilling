...
public class UserTest extends Base {
	...
	@Test(priority=1,description="Tc_006_verify_Add_User",groups={"Smoke"})
	public void Tc_006_verify_Add_User() {
		...
		String actEmail1 = user.getValidEmail();
		Assert.assertEquals(Email, actEmail1, ErrorMessages.RESET_PASSWORD_ERROR_MESSAGE);

	}
	@Test
	public void Tc_010_verify_Newly_Add_User() {
		List<ArrayList<String>> data = ExcelUtility.excelDataReader("LoginPage");
		String username = data.get(0).get(1);
		String password = data.get(1).get(1);
		String expUserName = data.get(2).get(1);
